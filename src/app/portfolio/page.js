...
const Page = () => {
    ...
    useEffect(() => {
        const fetchImages = async () => {
          try {
            const imageData = await client.fetch('*[_type == "work"]{title, description, "imageUrl": image.asset->url, gitLink}');
            setImages(imageData);
          } catch (error) {
    ...
    return (
        <div className='bg-white text-black min-h-screen h-1/2 bg-no-repeat'>
            ...
            <div className="grid grid-cols-1 mr-14 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 rounded-full h-1/6 gap-8 ml-20  mb-10 mt-10">
